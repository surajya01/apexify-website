...
const services = [
  ...
  { icon: LayoutDashboard, title: "Admin Dashboard",            description: "Custom analytics dashboards and back-office systems with real-time data visualization and role-based access.", color: "from-indigo-500 to-indigo-600",   tags: ["React", "Charts", "REST API"] },
  { icon: Code2,           title: "Custom Software",            description: "End-to-end bespoke software solutions tailored to your unique business workflows and operational requirements.", color: "from-violet-500 to-purple-600",   tags: ["Node.js", "Laravel", "PostgreSQL"] },
...
export default function Services() {
  ...
  return (
    <section id="services" ref={ref as React.RefObject<HTMLElement>} className="py-16 lg:py-28 bg-white" aria-label="Apexify Services">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-3xl mx-auto mb-12 lg:mb-16 reveal">
          ...
          <p className="section-subtitle mt-4 mx-auto">From concept to deployment, we provide end-to-end digital services that help your business innovate, automate, and scale.</p>
        </div>
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5 lg:gap-6">
          {services.map(({ icon: Icon, title, description, color, tags }) => (
            <TiltCard key={title} className="service-card group relative overflow-hidden reveal">
              ...
              <div className={`w-12 h-12 lg:w-14 lg:h-14 rounded-2xl bg-gradient-to-br ${color} flex items-center justify-center mb-4 lg:mb-5 group-hover:scale-110 transition-transform duration-300 shadow-lg`}>
              ...
              <div className="flex flex-wrap gap-1.5 mb-4">
                {tags.map((tag) => (
                  <span key={tag} className="text-xs bg-slate-50 text-slate-600 border border-slate-100 px-2.5 py-1 rounded-lg font-medium">{tag}</span>
                ))}
              </div>
              <button onClick={() => document.querySelector("#contact")?.scrollIntoView({ behavior: "smooth" })}
                className="flex items-center gap-2 text-blue-600 text-sm font-semibold group/btn"
                aria-label={`Learn more about ${title}`}>
                Learn More <ArrowRight size={15} className="group-hover/btn:translate-x-1 transition-transform" />
              </button>
            </TiltCard>
          ))}
        </div>

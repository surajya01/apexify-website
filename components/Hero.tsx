...
function StatCounter({ target, suffix = "", label, icon: Icon, color }: {
  ...
  return (
    <div ref={ref as React.RefObject<HTMLDivElement>} className="glass-card rounded-2xl p-4 lg:p-5 flex items-center gap-3 lg:gap-4 hover:bg-white/90 transition-all duration-300 group">
      <div className={`w-10 h-10 lg:w-12 lg:h-12 bg-gradient-to-br ${color} rounded-xl flex items-center justify-center flex-shrink-0 group-hover:scale-110 transition-transform`}>
        <Icon size={20} className="text-white" />
      </div>
      <div>
        <div className="text-xl lg:text-2xl font-bold text-slate-900">{count}{suffix}</div>
  ...
  return (
    <section id="home" ref={sectionRef} onMouseMove={onMouseMove}
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-1/4 -left-10 lg:-left-20 w-[300px] lg:w-[500px] h-[300px] lg:h-[500px] bg-blue-600/20 rounded-full blur-[80px] lg:blur-[120px] animate-blob" />
        <div className="absolute top-1/3 right-0 w-[250px] lg:w-[400px] h-[250px] lg:h-[400px] bg-cyan-400/15 rounded-full blur-[60px] lg:blur-[100px] animate-blob animation-delay-2000" />
        <div className="absolute bottom-0 left-1/2 w-[300px] lg:w-[450px] h-[300px] lg:h-[450px] bg-blue-800/20 rounded-full blur-[80px] lg:blur-[120px] animate-blob animation-delay-4000" />
        <div className="absolute inset-0 opacity-[0.03]"
          style={{ backgroundImage: "radial-gradient(circle, rgba(255,255,255,1) 1px, transparent 1px)", backgroundSize: "32px 32px" }} />
      </div>

      <div className="relative w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-28 pb-16 lg:py-24">
        <div className="grid lg:grid-cols-2 gap-10 lg:gap-14 items-center">

          {/* Left content */}
          <div className="text-white space-y-5 lg:space-y-7">
            {/* Badge */}
            <div className="inline-flex items-center gap-2 bg-white/10 backdrop-blur-sm border border-white/20 text-white/90 text-xs lg:text-sm font-medium px-3 lg:px-4 py-1.5 lg:py-2 rounded-full">
              <Sparkles size={12} className="text-cyan-400" />
              Full-Service Digital Agency · Est. 2019
            </div>

            {/* Headline */}
            <h1 className="text-4xl sm:text-5xl lg:text-6xl xl:text-[68px] font-extrabold leading-[1.08] tracking-tight">
              We Build<br />
              <span className="relative inline-block">
                <span className="bg-gradient-to-r from-cyan-300 via-blue-200 to-cyan-300 bg-clip-text text-transparent inline-block" style={{ minWidth: "8ch" }}>
                  {typed}
            ...
            <div className="flex flex-wrap gap-2">
              {["Free Consultation", "Transparent Pricing", "On-Time Delivery", "24/7 Support"].map((t) => (
                <span key={t} className="flex items-center gap-1.5 text-xs text-white/80 bg-white/10 border border-white/15 px-2.5 lg:px-3 py-1.5 rounded-lg">
                  <CheckCircle size={11} className="text-cyan-400" />{t}
                </span>
              ))}
            </div>

            {/* CTAs */}
            <div className="flex flex-wrap gap-3 lg:gap-4 pt-1">
              <button onClick={() => scrollTo("#contact")}
                className="group relative overflow-hidden bg-white text-blue-700 font-bold px-6 lg:px-8 py-3.5 lg:py-4 rounded-xl shadow-xl hover:-translate-y-0.5 transition-all duration-300 flex items-center gap-2">
                <span className="relative z-10 text-sm lg:text-base">Get Free Consultation</span>
              ...
              <button onClick={() => scrollTo("#portfolio")}
                className="group flex items-center gap-2 lg:gap-3 text-white/90 hover:text-white font-semibold px-5 lg:px-6 py-3.5 lg:py-4 rounded-xl border border-white/20 hover:border-white/40 hover:bg-white/10 transition-all duration-300 text-sm lg:text-base">
                View Our Work
            ...
            <div className="relative bg-slate-900/90 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl overflow-hidden">
              ...
              <div className="p-6 font-mono text-sm space-y-1">
                ...
                <div className="flex leading-7">
                  ...
                  <span className="inline-block w-2 h-4 bg-cyan-400 animate-blink mt-1" />
                </div>
              </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 pointer-events-none">
        <svg viewBox="0 0 1440 60" className="w-full fill-white" preserveAspectRatio="none" aria-hidden="true">

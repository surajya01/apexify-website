...
export default function Process() {
  ...
  return (
    <section id="process" ref={ref as React.RefObject<HTMLElement>} className="py-16 lg:py-28 bg-section-gradient" aria-label="Our development process">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-3xl mx-auto mb-12 lg:mb-16 reveal">
          ...
          <p className="section-subtitle mt-4 mx-auto">Our proven 6-step process ensures every project is delivered on time, within budget, and exceeds expectations.</p>
        </div>
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5 lg:gap-6">
          {steps.map(({ step, icon: Icon, title, description, color, duration }) => (
            <article key={title} className="relative bg-white rounded-2xl p-6 lg:p-7 border border-slate-100 shadow-sm hover:shadow-xl hover:-translate-y-1.5 transition-all duration-500 group reveal">
              <div className="flex items-center justify-between mb-5 lg:mb-6">
                <div className={`w-12 h-12 lg:w-14 lg:h-14 rounded-2xl bg-gradient-to-br ${color} flex items-center justify-center shadow-lg group-hover:scale-110 group-hover:rotate-3 transition-all duration-300`}>
                  <Icon size={24} className="text-white" strokeWidth={1.8} />
                </div>
                <span className="text-5xl lg:text-6xl font-black text-slate-100 select-none">{step}</span>
              </div>
              <h3 className="text-lg lg:text-xl font-bold text-slate-900 mb-2 group-hover:text-blue-600 transition-colors">{title}</h3>

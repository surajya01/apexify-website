...
export default function Portfolio() {
  ...
  return (
    <section id="portfolio" ref={ref as React.RefObject<HTMLElement>} className="py-16 lg:py-28 bg-white" aria-label="Apexify Portfolio">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-3xl mx-auto mb-10 lg:mb-12 reveal">
          ...
          <p className="section-subtitle mt-4 mx-auto">A curated selection of projects that showcase our expertise in building scalable, beautiful, and impactful digital products.</p>
        </div>

        {/* Filter tabs */}
        <div className="flex flex-wrap justify-center gap-2 mb-8 lg:mb-12 reveal" role="tablist" aria-label="Filter projects">
          {categories.map((cat) => (
            <button key={cat} onClick={() => setActive(cat)} role="tab" aria-selected={active === cat}
              className={`px-4 py-2 rounded-xl font-medium text-sm transition-all duration-200 ${
                active === cat ? "bg-blue-600 text-white shadow-lg scale-105" : "bg-slate-50 text-slate-600 hover:bg-blue-50 hover:text-blue-600 border border-slate-100"
              }`}>
              {cat}
            </button>
          ))}
              ...
              <div className="relative overflow-hidden h-44 sm:h-48 lg:h-52">
                ...
                <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300">
                  <button onClick={() => document.querySelector("#contact")?.scrollIntoView({ behavior: "smooth" })}
                    className="flex items-center gap-2 bg-white text-blue-700 font-semibold px-4 py-2.5 rounded-xl shadow-xl text-sm hover:bg-blue-50 transition-colors">
                    <ExternalLink size={15} />View Project
                  </button>
                </div>
                <div className="absolute top-3 left-3">
                  <span className="flex items-center gap-1.5 text-xs font-semibold bg-white/90 backdrop-blur-sm text-slate-700 px-2.5 py-1.5 rounded-lg shadow-sm">
                    <Tag size={10} />{project.category}
                  </span>
                </div>
              </div>
              <div className="p-5 lg:p-6">
                <h3 className="text-lg lg:text-xl font-bold text-slate-900 mb-2 group-hover:text-blue-600 transition-colors">{project.title}</h3>
